...
- If one signal group is clearly stronger, ask exactly one question:
  `Recommend <quick|deep> based on request signals. Choose mode: quick or deep.`
- If signals are mixed/unclear, ask exactly one question:
  `Pick planning mode: quick (3-4 turns) or deep (dynamic)?`
- Route:
  - quick → State 1Q
  - deep → State 1D
