...
name: skill-refine
description: >
  Produces iterative `.skill-plan.yaml` patches from structured feedback
  (skill-eval logs, skill-check reports, user guidance) with human approval.
  Outputs a patch artifact and a validated patched plan for skill-forge.
  Use when "refine skill", "improve plan", "patch plan", "apply eval feedback",
  or "/skill-refine".
---
# Skill Refine
